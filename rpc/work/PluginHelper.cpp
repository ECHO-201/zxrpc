...
void PluginHelper::load(const string &plugin)
{
    if (plugin.empty())
    {
        LOG_ERROR("load plugin failure: plugin is empty.");
        return;
    }
    if (m_plugins.find(plugin) != m_plugins.end())
    {
        LOG_DEBUG("plugin is already exist.");
        return;
    }
    ...
    if (!handle)
    {
        LOG_ERROR("load plugin failure: %s", dlerror());
        return;
    }
...
void PluginHelper::unload(const string &plugin)
{
    if (plugin.empty())
    {
        LOG_ERROR("unload plugin failure: plugin is empty.");
        return;
    }
    std::map<string, void *>::iterator it = m_plugins.find(plugin);
    if (it == m_plugins.end())
    {
        LOG_ERROR("unload plugin failure: plugin is not exist.");
        return;
    }
    dlclose(it->second);
    m_plugins.erase(it);
...
{   
    ...
    if (it == m_plugins.end())
    {
        ...
        if (it == m_plugins.end())
        {
            LOG_ERROR("load plugin failure: %s", plugin.c_str());
            return NULL;
        }
    }
    ...
    if (func == NULL)
    {
        LOG_ERROR("plugin: %s, undefined symbol: %s", plugin.c_str(), symbol.c_str());
    }
    return func;
}
...
void PluginHelper::show()
{
    ...
    {
        LOG_DEBUG("plugin: name=%s handle=%p", it->first.c_str(), it->second);
    }
}
